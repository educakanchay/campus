...
/*
 ...
 function autoattend_email2teachers_key($sess)
 function autoattend_email_text_attend($info)
 function autoattend_email_html_attend($info)
 function autoattend_email_text_key($info)
 function autoattend_email_html_key($info)
...
function autoattend_email_text_attend($info)
{
	$posttext  = $info->shortname.' ['.$info->date.']';
	$posttext .= "\n--------------------------------------------------------------------------\n\n";
	$posttext .= get_string('email_teacher_attend', 'block_autoattend', $info);
	$posttext .= "\n\n--------------------------------------------------------------------------\n";
...
function autoattend_email_html_attend($info)
{
	$posthtml  = '<h3>'.$info->shortname.'&nbsp;['.$info->date.']</h3><hr />'."\n";
	$posthtml .= '<strong>'.get_string('attenddata', 'block_autoattend').'</strong><br />'."\n";
	$posthtml .= '<font face="sans-serif"><p>'."\n";
	$posthtml .= get_string('email_teacher_attend', 'block_autoattend', $info)."\n";
	$posthtml .= '</p></font><hr />';
...
function autoattend_email2teachers_key($sess)
{
	...
	if (file_exists($CFG->dirroot.'/mod/autoattendmod/locallib.php')) {
		...
		if (function_exists('autoattendmod_send_email_teachers')) {
			...
			$posttext = autoattend_email_text_key($info);
			$posthtml = autoattend_email_html_key($info);
			$subject  = get_string('attendkey', 'block_autoattend').': '.$course->fullname.': '.$daytime;
...
function autoattend_email_text_key($info)
{
	$posttext  = $info->shortname.' ['.$info->date.']';
	$posttext .= "\n--------------------------------------------------------------------------\n\n";
	$posttext .= get_string('email_teacher_key', 'block_autoattend', $info);
	$posttext .= "\n\n--------------------------------------------------------------------------\n";
...
{
	$posthtml  = '<h3>'.$info->shortname.'&nbsp;['.$info->date.']</h3><hr />'."\n";
	$posthtml .= '<strong>'.get_string('attendkey', 'block_autoattend').'</strong><br />'."\n";
	$posthtml .= '<font face="sans-serif"><p>'."\n";
	$posthtml .= get_string('email_teacher_key_html', 'block_autoattend', $info)."\n";
	$posthtml .= '</p></font><hr />';
